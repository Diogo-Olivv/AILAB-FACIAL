    ...
    <div className="space-y-4">
      {days.map((day) => (
        <div key={day.key} className="overflow-hidden rounded-2xl border border-line bg-card">
          <div className="flex items-center justify-between gap-2 bg-navy px-4 py-3">
            <span className="text-sm font-medium capitalize text-white">{day.label}</span>
            <span className="whitespace-nowrap text-sm text-white/70">{formatDuration(day.totalSeconds)}</span>
          </div>
